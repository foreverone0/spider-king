...
class PublishApi:
    ...
    def __init__(self, username: str, password: str, question: Optional[int] = None, answer: Optional[str] = None):
        """
    ...
    def post(self,
             ...
             attachments: Optional[List[PublishPostAttachment]] = None) -> str:
        """
        发帖
        ...
        if attachments is not None:
            for i, attachment in enumerate(attachments):
                forms[f'attachment_{i}'] = (attachment.name, attachment.content, 'application/octet-stream')
        ...
        response = requests.post(uri, files=forms, headers=self.headers, cookies=self.cookies)
        response.raise_for_status()
